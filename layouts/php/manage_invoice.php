  ...
  function printInvoice($invoice_number) {
    ...
    <div class="row">
      ...
      <div class="col-md-4">
        ...
        <span class="font-weight-bold">Joy & Setu Pharmacy</span><br>
        <span class="font-weight-bold">Nakhalpara</span><br>
        <!-- <span class="font-weight-bold"><?php echo $p_email; ?></span><br> -->
        <span class="font-weight-bold">Mob. No.: 01722323264</span>
      </div>
      <div class="col-md-1"></div>
    </div>
    <div class="row text-center">
        ...
        <table class="table table-bordered table-striped table-hover" id="purchase_report_div">
          ...
          <tbody>
            <?php
              $seq_no = 0;
              $total = 0;
              $query = "SELECT * FROM sales_item INNER JOIN invoices ON sales_item.SALES_ID = invoices.SALES_ID INNER JOIN medicines ON sales_item.MEDICINE_ID = medicines.ID WHERE INVOICE_ID = $invoice_number";
              $result = mysqli_query($con, $query);
              while($row = mysqli_fetch_array($result)) {
                $seq_no++;
                ?>
